...
public class UserDaoImpl implements UserDao {
	...
	public UserDaoImpl() throws SQLException{
		...
		pst3 = connection.prepareStatement("INSERT INTO users (first_name, last_name, email, password, dob, status, role) VALUES (?, ?, ?, ?, ?, ?, ?);");
	...
	public String registerVoter(User newVoter) throws SQLException {
		...
		pst3.setString(1,newVoter.getFirstName());//fn
		pst3.setString(2,newVoter.getLastName());//ln
		pst3.setString(3,newVoter.getEmail());//em
		pst3.setString(4,newVoter.getPassword());//pass
		pst3.setDate(5, newVoter.getDob());//dob
		pst3.setBoolean(6, newVoter.isStatus());//voting status , false=> not yet voted !
		pst3.setString(7, newVoter.getUserRole());//role
