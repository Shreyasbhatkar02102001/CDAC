...
#include <cstdlib>
#include <algorithm>
#include "../nlohmann/json.hpp" // Include the nlohmann/json.hpp header for JSON

using namespace std;
using json = nlohmann::json;

// Function to get random meanings (3 random + 1 correct)
vector<string> getRandomMeanings(const json& data, const string& word, string& correctAnswer) {
    vector<string> allMeanings;  // List of all possible meanings
    vector<string> options;     // Final choices to show the player

    // Collect all meanings from other words
    for (auto& item : data.items()) {
        if (item.key() != word) {
            for (string meaning : item.value()) {
                allMeanings.push_back(meaning);
            }
        }
    }

    // Randomly pick 3 meanings
    while (options.size() < 3 && !allMeanings.empty()) {
        int randomIndex = rand() % allMeanings.size();
        options.push_back(allMeanings[randomIndex]);
        allMeanings.erase(allMeanings.begin() + randomIndex); // Remove used meaning
    }

    // Add the correct meaning
    correctAnswer = data[word][0];  // Get the correct meaning
...
int main() {
    ...
    // Load the word-meaning data from a file
    ...
    vector<string> words;
    for (auto& item : data.items()) {
        words.push_back(item.key());
    }

    // Shuffle words and pick 5 random ones
    random_shuffle(words.begin(), words.end());
    words.resize(5);

    int score = 0;  // Keep track of the player's score

    // Quiz the player on 5 words
    for (int i = 0; i < 5; i++) {
        string word = words[i];
        string correctAnswer;

        // Get the options for the current word
        vector<string> options = getRandomMeanings(data, word, correctAnswer);

        // Show the word and options
        cout << "*******************************\n";
        cout << "Word: " << word << endl;
        cout << "*******************************\n";
        char optionLabel = 'A';
        ...
        int selectedIndex = toupper(answer) - 'A';
        if (options[selectedIndex] == correctAnswer) {
            cout << "CORRECT!\n";
            score++;
        } else {
